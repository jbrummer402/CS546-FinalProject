...
async function create(firstName, lastName, dateOfBirth, username, password, address, photoLink, email) {
    // handle inputs
    const errorObj = await checkInputs(firstName, lastName, dateOfBirth, username, password, address, photoLink, email, 'POST');
    if (errorObj.error) {
...
async function readByUsername(username) {
    // handle inputs
    if (username === undefined) throw 'Input must be provided for \'username\' parameter!';
...
async function update(updateObj) {
    ...
    let {id, firstName, lastName, dateOfBirth, password, address, photoLink, email} = updateObj;
    let user = {};

    // check if id is valid
    if (id === undefined || typeof id !== 'string') throw "String input must be provided for 'id' parameter!";
    ...
    if (firstName === undefined && lastName === undefined && dateOfBirth === undefined && password === undefined &&
        ...
        throw "Input must be provided for at least one of the following parameters: 'firstName', 'lastName', 'dateOfBirth'," +
    ...
    try {
        await checkInputs(  firstName === undefined ? user.firstName : firstName,
    ...
    const updatedUser = {
        ...
        dateOfBirth: dateOfBirth === undefined ? new Date(user.dateOfBirth) : new Date(dateOfBirth),
        username: user.username,
        password: password === undefined ? user.password : await bcrypt.hash(password.trim(), saltRounds),
        address: address === undefined ? user.address : {  "street": address.street.trim(),
                                                            "aptNo": address.aptNo ? address.aptNo.trim() : '',
                                                            "zipCode": address.zipCode.trim(),
                                                            "state": address.state.trim().toUpperCase(),
                                                            "town": address.town.trim(),
...
module.exports = {create, readByID, readByUsername, remove, update, usernameExists, emailExists};